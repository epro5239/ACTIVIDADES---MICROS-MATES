...
// Datos de advertisement BLE: flags + nombre + UUID del servicio de temperatura ambiental
static uint8_t adv_data[] = {
    // Flags: general discoverable, BR/EDR not supported
    0x02, BLUETOOTH_DATA_TYPE_FLAGS, APP_AD_FLAGS,
    // Complete Local Name: "picow_temp" (10 chars)
    0x0B, BLUETOOTH_DATA_TYPE_COMPLETE_LOCAL_NAME,
          'p','i','c','o','w','_','t','e','m','p',
    // Complete List of 16-bit Service UUIDs: Environmental Sensing (0x181A)
...
void packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    switch(event_type){
        case BTSTACK_EVENT_STATE:
            ...
            printf("BTstack servidor activo en: %s\n", bd_addr_to_str(local_addr));
            ...
            printf("Advertisements habilitados. Esperando cliente...\n");
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            ...
            printf("Cliente desconectado. Re-anunciando...\n");
            // Re-enable advertisements after disconnection
            gap_advertisements_enable(1);
            break;

        case ATT_EVENT_CAN_SEND_NOW:
            // Enviar notificacion con temperatura actual
            ...
            printf("Notificacion enviada: %.2f degC\n", (float)current_temp / 100.0f);
            break;
...
uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle,
    ...
    if (att_handle == ATT_CHARACTERISTIC_ORG_BLUETOOTH_CHARACTERISTIC_TEMPERATURE_01_VALUE_HANDLE){
        return att_read_callback_handle_blob((const uint8_t *)&current_temp,
                                             sizeof(current_temp), offset, buffer, buffer_size);
    }
    return 0;
}
...
int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle,
    ...
    printf("Notificaciones %s\n", le_notification_enabled ? "HABILITADAS" : "deshabilitadas");
...
void poll_temp(void) {
    ...
    // Scale raw reading to 16-bit value
    ...
    // Vbe = 0.706V @ 27°C, slope = -1.721mV/°C
    float deg_c = 27.0f - (reading - 0.706f) / 0.001721f;
    current_temp = (uint16_t)(deg_c * 100.0f);
    printf("[Servidor] Temperatura: %.2f degC (raw: %u)\n", deg_c, raw32);
}